...
import com.skcraft.cardinal.util.ObjectCache;
...
public class PartyCache implements ObjectCache<String, Party> {
    ...
    private final LoadingCache<String, Party> cache = CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<String, Party>() {
                ...
                public Party load(String key) throws Exception {
                    Party party = manager.getAll(Lists.newArrayList(key)).get(key);
                    if (party != null) {
                        return party;
                    } else {
     ...
     */
    @Inject
    public PartyCache(PartyManager manager, EventBus eventBus) {
    ...
    public void refreshAll() {
        log.info("Reloading all parties...");
        manager.refreshAll(cache.asMap().values()).forEach(s -> cache.invalidate(s.toLowerCase()));
    }
    ...
    public void removeMembers(Party party, Set<Member> members) {
        ...
        Set<MojangId> ids = members.stream().map(Member::getUserId).collect(Collectors.toSet());
